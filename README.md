# 📚 AI Student Notes Summarizer & Quiz Generator

An AI-powered, single-page web application designed for students to instantly parse lengthy study materials, extract structured key points, and test their comprehension with automated practice quizzes in real-time.

---

## 🌟 Key Features

- **⚡ Instant AI Summarization:** Converts complex notes and long text into clear, digestible bullet points using Google Gemini AI.
- **🎯 Dynamic Quiz Generation:** Automatically creates 3 multiple-choice practice questions with instant grading and clear explanations to reinforce learning.
- **💡 Smart Mock Fallback:** Operates seamlessly with intelligent fallback content even without an API key, ensuring zero downtime during testing.
- **🎨 Modern UI/UX:** Clean, distraction-free interface built with Tailwind CSS, featuring a fully integrated Dark/Light mode toggle.
- **🔑 Client-Side Privacy:** Your Gemini API Key is stored safely in your browser's `localStorage` and is never sent to external servers.

---

## 🛠️ Tech Stack

- **Frontend:** HTML5, Tailwind CSS
- **Logic & Scripting:** JavaScript (ES6+)
- **AI Integration:** Google Gemini API (`gemini-3.6-flash`)
- **Storage:** Browser `localStorage`

---

## 🚀 Getting Started Locally

1. **Clone the Repository:**
   ```bash
   git clone [https://github.com/nachiar187/AI-Student-Notes-Summarizer-Quiz-Generator.git]
